...
def create_agents(chat: OpenAIChatClient):
    # No model here; model_id is already set on the client
    triage = chat.create_agent(
        name="triage_agent",
        instructions=(
            "Read the latest user message and decide whether to hand off to "
...
async def drain(stream: AsyncIterable[WorkflowEvent]):
...
def handle(events):
    reqs = []
    for e in events:
        if isinstance(e, WorkflowStatusEvent) and e.state in {
            WorkflowRunState.IDLE, WorkflowRunState.IDLE_WITH_PENDING_REQUESTS
        }:
...
async def main():
    # IMPORTANT: use model_id (not model) for this AF build
    ...
    wf = (
        HandoffBuilder(name="customer_support", participants=[triage, refund, order, support])
